...
import { useJob, useUpdateJob, useMessages, useCreateMessage } from "@/hooks/use-data";
...
import { Loader2, Send, MapPin, Calendar, User, FileText, CheckCircle } from "lucide-react";
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p>{job.createdAt ? format(new Date(job.createdAt), "PPP") : "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Last Updated</p>
                    <p>{job.updatedAt ? format(new Date(job.updatedAt), "PPP") : "N/A"}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Assigned Ops</p>
                    <div className="flex items-center gap-2 mt-1">
                      <User className="h-4 w-4 text-muted-foreground" />
                      <span>{job.assignedOps?.firstName || "Unassigned"}</span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <User className="h-4 w-4 text-muted-foreground" />
                      <span>{job.assignedEngineer?.firstName || "Unassigned"}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
